...
const startCreating = async () => {
  ...
  debugLogs
    ? console.log("Total Editions to create: ", abstractedIndexes)
    : null;
  while (layerConfigIndex < layerConfigurations.length) {
    while (
      editionCount <= layerConfigurations[layerConfigIndex].growEditionSizeTo
    ) {
	  console.log("editionCount: ", editionCount)//KI_deepa
	  if(editionCount%2){
      ...
      if (isDnaUnique(dnaList, newDna)) {
        ...
        if (failedCount >= uniqueDnaTorrance) {
		  ...
		  const layers = layersSetup(
      MalelayerConfigurations[layerConfigIndex].layersOrder
    );
      let newDna = createDna(layers);
      if (isDnaUnique(dnaList, newDna)) {
        let results = constructLayerToDna(newDna, layers);
        ...
        await Promise.all(loadedElements).then((renderObjectArray) => {
          ...
          // if there's a prefix for the current configIndex, then
          // start count back at 1 for the name, only.
          const _prefix = MalelayerConfigurations[layerConfigIndex].namePrefix
            ? MalelayerConfigurations[layerConfigIndex].namePrefix
            : null;
          // if resetNameIndex is turned on, calculate the offset and send it
          // with the prefix
          let _offset = 0;
          if (MalelayerConfigurations[layerConfigIndex].resetNameIndex) {
            _offset = MalelayerConfigurations.reduce((acc, layer, index) => {
              if (index < layerConfigIndex) {
                acc += layer.growEditionSizeTo;
                return acc;
              }
        ...
        if (failedCount >= uniqueDnaTorrance) {
          console.log(
            `You need more layers or elements to grow your edition to ${MalelayerConfigurations[layerConfigIndex].growEditionSizeTo} artworks!`
          );
          process.exit();
        }
      }//if (isDnaUnique(dnaList, newDna))
